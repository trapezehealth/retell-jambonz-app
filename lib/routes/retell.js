...
const service = ({ logger, makeService, activeJambonzSessions }) => { // activeJambonzSessions passed from app.js
  ...
  svc.on('session:new', async (session) => {
    ...
    session.locals.logger.info({"sbc_callid": session.sbc_callid, call_sid: session.call_sid}, 'session started');
    let { from } = session;
    session.locals.logger.debug({ session }, `New Jambonz session: ${session.sbc_callid}`);

    // Store session for external control via HTTP webhook
    activeJambonzSessions.set(session.sbc_callid, session);
    session.locals.logger.debug(`Session ${session.sbc_callid} added to activeJambonzSessions. Map size: ${activeJambonzSessions.size}`);
    ...
    if (session.direction === 'inbound'  &&
      ...
      if (username === env.sipUsername) {
        session.locals.logger.debug(`call ${session.sbc_callid} is coming from Retell`);
...
const onRefer = (session, evt) => {
  ...
  logger.debug({ refer_details }, `session ${session.sbc_callid} received refer`);
...
const onClose = (session, activeJambonzSessions, code, reason) => {
  ...
  logger.info({ code, reason }, `Jambonz session ${session.sbc_callid} and call_sid ${session.call_sid} closed`);
};
